...
class SiteController extends Controller
{
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate())
        {
            if ($model->sendEmail(Yii::$app->params['adminEmail']))
            {
                Yii::$app->session->setFlash(
                    'success', 
                    Yii::t('messages', 'Thank you for contacting us. We will respond to you as soon as possible.')
                );
            } 
            else 
            {
                Yii::$app->session->setFlash(
                    'error', 
                    Yii::t('messages', 'There was an error sending your message.')
                );
            }
    ...
    public function actionRequestPasswordReset()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate())
        {
            ...
            Yii::$app->session->setFlash(
                'error', 
                Yii::t('messages', 'Sorry, we are unable to reset password for the provided email address.')
            );            
        }
    ...
    public function actionResetPassword($token)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save())
        {
            Yii::$app->session->setFlash(
                'success', 
                Yii::t('messages', 'New password saved.')
            );
    ...
    public function actionVerifyEmail($token)
    {
        ...
        if ($user = $model->verifyEmail())
        {
            if (Yii::$app->user->login($user))
            {
                Yii::$app->session->setFlash(
                    'success', 
                    Yii::t('messages', 'Your email has been confirmed!')
                );
    ...
    public function actionResendVerificationEmail()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate())
        {
            ...
            Yii::$app->session->setFlash(
                'error', 
                Yii::t('messages', 'Sorry, we are unable to resend verification email for the provided email address.')
            );
        }
